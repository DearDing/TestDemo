...
public class ExTextView extends AppCompatTextView {
    private static final String TAG = ExTextView.class.getSimpleName();
    ...
    public void setOriginalText(CharSequence originalText) {
        ...
        if (maxLines != -1) {
            ...
            if (mExpandable) {
                ...
                //计算原文截取位置
                int endPos = layout.getLineEnd(maxLines);
...
//                    }
    ...
    private void open() {
        if (hasAnimation) {
            ...
            ExTextView.super.setMaxLines(Integer.MAX_VALUE);
            setText(mOpenSpannableStr);
    ...
    private void close() {
        if (hasAnimation) {
            ...
            ExTextView.super.setMaxLines(mMaxLines);
            setText(mCloseSpannableStr);
            if (mOpenCloseCallback != null) {
