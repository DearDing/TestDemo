...
import com.ding.kotlin.expandable.ExpandableTextView;
import com.ding.kotlin.expandable.LinkType;
import com.ding.kotlin.expandable.StatusType;
...
public class TextView1Activity extends AppCompatActivity {
    ...
    private ExpandableTextView.OnLinkClickListener linkClickListener = (type, content, selfContent) -> {
        if (type.equals(LinkType.LINK_TYPE)) {
            Toast.makeText(TextView1Activity.this, "你点击了链接 内容是：" + content, Toast.LENGTH_SHORT).show();
        } else if (type.equals(LinkType.MENTION_TYPE)) {
            Toast.makeText(TextView1Activity.this, "你点击了@用户 内容是：" + content, Toast.LENGTH_SHORT).show();
        } else if (type.equals(LinkType.SELF)) {
            Toast.makeText(TextView1Activity.this, "你点击了自定义规则 内容是：" + content + " " + selfContent, Toast.LENGTH_SHORT).show();
        }
    };
    ...
    private void setContent(String yourText, boolean d) {
        ...
        views[1].setExpandOrContractClickListener(type -> {
            if (type.equals(StatusType.STATUS_CONTRACT)) {
                Toast.makeText(TextView1Activity.this, "收回操作", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(TextView1Activity.this, "展开操作", Toast.LENGTH_SHORT).show();
            }
        });
        //添加展开和收回操作 只触发点击 不真正触发展开和收回操作
        views[11].setExpandOrContractClickListener(type -> {
            if (type.equals(StatusType.STATUS_CONTRACT)) {
                Toast.makeText(TextView1Activity.this, "收回操作，不真正触发收回操作", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(TextView1Activity.this, "展开操作，不真正触发展开操作", Toast.LENGTH_SHORT).show();
            }
        },false);
        ...
        views[10].setOnGetLineCountListener(new ExpandableTextView.OnGetLineCountListener() {
            ...
            public void onGetLineCount(int lineCount, boolean canExpand) {
                Toast.makeText(TextView1Activity.this, "行数：" + lineCount + "  是否满足展开条件：" + canExpand, Toast.LENGTH_SHORT).show();
            }
        });
    }
